...
import {
  ...
  Settings, ShieldCheck, Trash2, LogOut, X, User, Moon
} from 'lucide-react';
    ...
    <>
      <div className={`bg-[#171717] h-screen transition-all duration-300 border-r border-white/10 flex flex-col ${isOpen ? 'w-64' : 'w-0 overflow-hidden'}`}>
        ...
        <div className="flex-1 flex flex-col min-h-0 p-4">
          ...
          {/* ✅ New Chat Button - Consistent Theme */}
          <button
            onClick={() => {
              setActiveThreadId(null);
              navigate('/chat');
            }}
            className="flex items-center gap-3 p-3 mb-6 rounded-xl text-white shrink-0 transition-all active:scale-95
                       bg-[#2f2f2f] hover:bg-[#383838] border border-white/5 shadow-md group"
          >
                ...
                >
                  ...
                  {confirmDeleteId === thread.thread_id ? (
                    <div className="flex items-center gap-2 flex-1">
                      ...
                      <button onClick={(e) => handleConfirmDelete(e, thread.thread_id)} className="text-[10px] bg-red-500 hover:bg-red-600 text-white px-2 py-0.5 rounded-md">Yes</button>
                      <button onClick={handleCancelDelete} className="text-[10px] bg-[#444] text-white px-2 py-0.5 rounded-md">No</button>
                    </div>
                  ) : (
                    <>
                      <span className="truncate flex-1">{thread.name}</span>
                      <button onClick={(e) => handleDeleteClick(e, thread.thread_id)} className="opacity-0 group-hover:opacity-100 p-1 hover:bg-red-500/20 rounded-md text-gray-500 hover:text-red-400 transition-all">
                        <Trash2 size={13} />
                      </button>
                    </>
                  )}
        ...
        <div className="p-4 border-t border-white/5 bg-[#171717] shrink-0 space-y-1">
          ...
          <div
            ...
            className="p-3 text-sm hover:bg-[#2f2f2f] rounded-lg cursor-pointer flex items-center gap-3 text-gray-400 hover:text-white transition-colors group"
          >
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[999] flex items-center justify-center p-4">
          <div className="bg-[#1e1e1e] border border-white/10 w-full max-w-md rounded-2xl shadow-2xl animate-in fade-in zoom-in duration-200">
            <div className="p-6">
              ...
              <div className="space-y-3">
                <div 
                  ...
                  className="flex items-center justify-between p-4 bg-[#2f2f2f] hover:bg-[#383838] rounded-xl border border-white/5 cursor-pointer transition-colors"
                >
                  <div className="flex items-center gap-3">
                    <User size={18} className="text-gray-400" />
                    <span className="text-sm text-white font-medium">Profile Settings</span>
                  </div>
                  <span className="text-[10px] text-blue-400 font-bold uppercase">Manage</span>
                </div>

                <div className="flex items-center justify-between p-4 bg-[#2f2f2f] rounded-xl border border-white/5">
                  <div className="flex items-center gap-3">
                    <Moon size={18} className="text-gray-400" />
                    <span className="text-sm text-white font-medium">Appearance</span>
                  </div>
                  <span className="text-[10px] text-gray-500 font-bold uppercase">Dark Mode</span>
                </div>
              </div>

              <button 
                onClick={() => setShowSettings(false)}
                className="w-full mt-10 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-blue-600/20 active:scale-[0.98]"
              >
                Close Settings
              </button>
            </div>
